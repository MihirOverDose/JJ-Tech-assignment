...
//done - still remaining some part to understand
const search = async () => {
  const categoryId = (
    await apiRoot.categories().withKey({ key: "c2" }).get().execute()
  ).body.id;
  ...
  const productProjectionPagedQueryResult = (await simulateSearch(searchParams))
    .body;
  ...
  log(
    products.map(
      (product) =>
        `${countMatchingVariantsIn(product)} matching variant(s) in ${
          product.key
        }`
    )
  );
...
const formatTermFields = ({ count, term }: FacetTerm, facet: string) =>
