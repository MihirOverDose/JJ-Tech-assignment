...
export const createImportContainer = (
  ...
  //resource type:  [category, order, order-patch, price, product, product-draft, product-type, product-variant, product-variant-patch, customer, inventory, standalone-price, type]
...
export const checkImportOperationStatusById = (
  ...
  return importApiRoot.importOperations().withIdValue({ id }).get().execute();
};

export const importProductDrafts = async (
  importContainerKey: string
): Promise<ClientResponse<ImportResponse>> => {
  const productDraft = await createProductDraftImportRequest();
  return importApiRoot
    .productDrafts()
    .importContainers()
    .withImportContainerKeyValue({ importContainerKey })
    .post({
      body: {
        resources: productDraft.resources,
